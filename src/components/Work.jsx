            ...
            <ul className={styles['work-list']}>
                {popupData.map((popup) => (
                    <li className={styles['work-item']} key={popup.id}>
                        <button type="button" className={`${styles['work-popup-button']} ${styles[popup.class]}`} onClick={() => openPopup(popup.id)} aria-haspopup="true" aria-controls={popup.class} aria-expanded={popup.id === activePopup}>
